...
/*   Updated: 2024/12/18 23:49:20 by oel-moue         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	double x, y;
	x = calcule_first_intersection_with_x(data) * SQUAR_SIZE;
	...
	data->raycas->distance_horizontal = sqrt(pow(x - data->player->player_x * SQUAR_SIZE, 2)
			+ pow(y - data->player->player_y * SQUAR_SIZE, 2));
}

void	distance_vertical(t_data *data)
{
	double x, y;
	x = calcule_first_intersection_with_y(data) * SQUAR_SIZE;
	...
	data->raycas->distance_vertical = sqrt(pow(x - data->player->player_x *SQUAR_SIZE, 2)
			+ pow(y - data->player->player_y *SQUAR_SIZE, 2));
}
...
void	cast_rays(t_data *data)
{
	...
	while (ray < NUM_RAYS)
	{
		...
		cast_ray(data);
		draw_line(data->player->player_x * SQUAR_SIZE, data->player->player_y * SQUAR_SIZE,
